 ...
 class u_instr_packet extends instr_packet;
  ...
  class j_instr_packet extends instr_packet;
    ...
    constraint fields {
      ...
      rs1 == 0;
      rs2 == 0;
      imm dist {[-(2**20):-1] :/ 5, [0:(2**6)-1] :/ 90, [(2**6):(2**20)-1] :/5};
...
function automatic [`INSTR_WIDTH-1:0] generate_instr(
  ...
  unique case (instr_type)
    ...
    `L_INSTR: begin
      ...
      instr_word[`RD_RANGE]     = rd;
      instr_word[`OPCODE_RANGE] = opcode;
      $display("DEBUG I ENCODE: imm_i_s=%0d, rd=%0d, rs1=%0d, funct3=%0d -> instr=%h", imm_i_s, rd, rs1, funct3, instr_word);
    end
    `S_INSTR: begin
    ...
    `J_INSTR: begin
      ...
      instr_word[`OPCODE_RANGE] = opcode;
      /// debug
      $display("DEBUG FLORIN IMM: imm = %h, imm[20] = %h, imm[19:12] = %h, imm[11] =%h, imm[10:1] = %h",imm[31:0],imm_j[20],imm_j[19:12],imm_j[11],imm_j[10:1]);
      $display("DEBUG FLORIN INSTR: instr = %h, instr[31] = %h, instr[30:21] = %h, instr[20] =%h, instr[19:12] = %h",instr_word[31:0],instr_word[31],instr_word[30:21],instr_word[20],instr_word[19:12]);    
    end
    `N_INSTR: begin
      instr_word = '0;
    end
...
task generate_n_instr_to_hex(string filepath, int n); // Just a repeater 
  ...
  for (i = 0; i < n; i++) begin
      ...
      int target_pc;
      int offset;
      choice = $urandom_range(0, 16); // 0 to 6
      case (choice)
         0,1,2,3:  begin r_pkt = new(); pkt = r_pkt; end     
         4,5,6,7:  begin i_pkt = new(); pkt = i_pkt; end
         8,13,14,15:        begin l_pkt = new(); pkt = l_pkt; end
         9:        begin s_pkt = new(); pkt = s_pkt; end
    ...
    //    target_pc  = target_idx * `PC_INCR;
      ...
      if ((pkt.instr_type == `L_INSTR) || (pkt.instr_type == `S_INSTR)) begin
        ...
        if (align_bits != 0) begin
          if (!allow_unaligned_ls) begin
            pkt.imm &= ~mask;  // force aligned offset
          end else begin
            if ($urandom_range(0,1)) begin
              pkt.imm &= ~mask;                                      // still generate aligned case
            end else begin
              int misalign = $urandom_range(1, mask);
              pkt.imm = (pkt.imm & ~mask) | misalign;                 // deliberately misalign
            end
          end
        end
      end
