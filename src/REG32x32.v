...
`ifdef SIMULATION_ON
  initial begin
    $readmemh("/home/fananiae/disertatie_Anania/src/reg_init.hex", Reg_Cell);
    Reg_Cell[0][`DATA_WIDTH-1:0] = {`DATA_WIDTH{1'b0}};  // Keep x0 hardwired to zero.
  end
`endif
...
`else
      Reg_Cell[0][`DATA_WIDTH-1:0] <= {`DATA_WIDTH{1'b0}};
`endif
